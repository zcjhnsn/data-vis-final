...
When we presented in class, we mentioned some things that needed to be ironed out. Updating the year didn't update the all of the visualizations. The parallel set viz needed more work than the other visualizations. It had a weird error that would display undefined as a category when a state was selected twice and the diverse dataset made the parallel set split into very thin lines that were difficult to read and interpret. The times in the treemap were difficult to read when a state with many fatalities was selected (i.e. Texas and California). The times were removed when a state had more than 100 crashes just to make the treemap easier to read. After those bugs and improvements were made, we made a legend for the treemap. We needed a legend that would convey the importance of position, color and size. The legend shows that top left position, large size, and darker red indicate more casualties. Implementing this was a throwback to our early assignments drawing shapes with d3.

Our site doesn't have the world's best design, but the site works on all platforms (including mobile) which is pretty cool.
